...
// TweetAnalysisService interface for dependency injection
type TweetAnalysisService interface {
	AnalyzeNewTweet(ctx context.Context, tweet *model.Tweet)
}

// dbWorkerWithRAG extends the regular worker with database integration and RAG analysis
type dbWorkerWithRAG struct {
	db *sqlx.DB
...
func NewDBWorkerWithRAG(
	db *sqlx.DB,
	twitterClientManager *twitterclient.Manager,
	heapHelper HeapHelper,
	tweetAnalysisService TweetAnalysisService,
) *dbWorkerWithRAG {
	return &dbWorkerWithRAG{
		db:                   db,
...
func (w *dbWorkerWithRAG) ProduceFromHeapToTweetChanWithDB(
	ctx context.Context,
	...
	for !heap.Empty() && ctx.Err() == nil {
		...
		logger.
			...
			Infoln("processing user from heap with database integration and RAG analysis")
	...
	logger.
		WithField("worker", "producer").
		Info("all producers finished successfully for SimpleWorker with DB and RAG")
	return unsentTweets, nil
}

func (w *dbWorkerWithRAG) fetchTweetOrFallbackToHeapWithDB(
	ctx context.Context,
	cancel context.CancelCauseFunc,
	...
	logger.WithField("user", entity.Name()).Infoln("fetching user tweets with database integration and RAG analysis")
	...
	logger.
		...
		Infoln("found tweets, saving to database with RAG analysis and preparing to push to tweet channel")

	// Save tweets to database before processing (now includes RAG analysis)
...
// saveTweetsToDatabase saves tweets to the database and performs RAG analysis
func (w *dbWorkerWithRAG) saveTweetsToDatabase(
	ctx context.Context,
	...
	for _, tweet := range tweets {
		...
		// Perform RAG analysis on the newly saved tweet
		if w.tweetAnalysisService != nil {
			w.tweetAnalysisService.AnalyzeNewTweet(ctx, dbTweet)
		}
	}
}

// DownloadTweetMediaFromTweetChanWithDB handles downloading of tweets with database integration
func (w *dbWorkerWithRAG) DownloadTweetMediaFromTweetChanWithDB(
	ctx context.Context,
	cancel context.CancelCauseFunc,
...
func (w *dbWorkerWithRAG) downloadTweetMediaWithDB(
	ctx context.Context,
	...
	for i, url := range urls {
		filename := fmt.Sprintf("%d_%d_%s", tweet.Id, i, filepath.Base(url))
	...
	for i, url := range validUrls {
		...
		if err != nil {
			...
			// Remove failed media record (but don't fail the whole operation)
			logger.
				WithFields(log.Fields{
					"media_id": mediaRecord.Id,
				}).
				Warn("media record created but download failed, keeping record for manual cleanup")
			continue
		}
		...
		logger.
			...
			Debug("successfully downloaded media")
	}
